    ...
    if request.headers.get('Content-Type') == 'application/json':
        ...
        if 'message' in data:
            ...
            if acquire_lock('locked'):
                threading.Thread(target=pubsub_handler, args=(gmail_service,drive_service,sheets_service,payload)).start()
...
def reset_watch():
    ...
    gmail_service = build('gmail', 'v1', credentials=g.creds)
    gmail_service.users().watch(userId='me', body=watch_request).execute() #Inicializa el modo watch de la API de Gmail así que solo debo llamarlo cada <7 días.
